...
对于单轴晶体，$\epsilon_x=\epsilon_y=n_o^2$，$\epsilon_z=n_e^2$，将$\vec{k}=k(\sin\theta,0,\cos\theta),\vec{S}=k(\sin\xi,0,\cos\xi)$代入方程可得：
$$\begin{aligned}
...
$$\delta_{oe}=\frac{2\pi}{\lambda}(n_o-n_e)d$$
控制材料的长度，即可得到不同的相位延迟片。对于负晶体，$n_e < n_o$，所以相位延迟片使得e光相位超前；对于正晶体，$n_e > n_o$，所以相位延迟片使得e光相位落后。
...
电场为：
$$E_x=A_x\cos(\omega t),E_y=\pm A_y\sin(\omega t)$$
假设x方向为o光，对于负晶体，e光相位超前，对应$E_y=- A_y\sin(\omega t)$，是右旋椭圆偏振光；对于正晶体，e光相位落后，对应$E_y=+ A_y\sin(\omega t)$，是左旋椭圆偏振光。
