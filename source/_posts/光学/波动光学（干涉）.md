...
当倾角$\alpha$增大的时候，条纹间距变小，劈尖处的条纹固定，所以条纹会向劈尖处移动。由于半波损失，劈尖处为暗条纹。
...
$$\begin{aligned}
I &\propto \int_{-\Delta x/2}^{+\Delta x/2}\cos^2{\left(\frac{kd}{2D}(x+\frac{D}{R}\xi)\right)}d\xi\\
&=[1+\text{sinc}{(\frac{k d\Delta x}{2R})}\cos{(\frac{k d}{ D}x)}]\\
\end{aligned}$$

$$\begin{aligned}
I &\propto \int_{-\Delta k/2}^{+\Delta k/2}\cos^2{\left(\frac{dx}{2D}(k_0+k)\right)}dk\\
