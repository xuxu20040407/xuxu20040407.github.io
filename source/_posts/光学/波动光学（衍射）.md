...
&=\frac12(1+\frac{(R+b)^2-R^2-(b+\frac{k\lambda}{2})^2}{2R(b+\frac{k\lambda}{2})})\\
&\approx 1-\frac{k\lambda}{4}(\frac1R+\frac1b)
\end{aligned}$$
代入$\lambda\sim 100nm,b\sim 1m,R\sim 1m$可知，振幅随着波带数的变化及其缓慢。

做题的时候，只需要知道以下公式：
- 最大波带半径：$r_{max}=\sqrt{b^2+R(R+b)\sin^2\alpha}=b+k\frac{\lambda}{2}$
...
\end{aligned}$$
所以光强变为：
$$I=I_0(\frac{\sin \alpha}{\alpha})^2(\frac{\sin \beta}{\beta})^2,\alpha=\frac{\pi a \sin \theta_1 }{\lambda},\beta=\frac{\pi b \sin \theta_2 }{\lambda}$$
...
第一个零点的位置为：
$$R\theta\approx0.61\lambda$$
或表示为直径的更熟悉的形式：
$$D\theta\approx1.22\lambda$$
...
&=\sum_{n=1}^{N}A\exp{[i(n-1)kd\sin\theta]}\text{sinc}(\alpha)\\
&=A\text{sinc}(\alpha)\sum_{n=1}^{N}\exp{[i(n-1)kd\sin\theta]}\\
&=A\text{sinc}(\alpha)\frac{1-\exp{[iNkd\sin\theta]}}{1-\exp{[ikd\sin\theta]}}\\
&=A\text{sinc}(\alpha)\frac{\exp{[i\frac N2kd\sin\theta]}}{\exp{[i\frac12kd\sin\theta]}}\times\frac{\exp{[-i\frac N2kd\sin\theta]}-\exp{[i\frac N2kd\sin\theta]}}{\exp{[-i\frac12kd\sin\theta]}-\exp{[i\frac12kd\sin\theta]}}\\
&=A\text{sinc}(\alpha)\frac{\exp{[i\frac N2kd\sin\theta]}}{\exp{[i\frac12kd\sin\theta]}}\times\frac{\sin{N\beta}}{\sin{\beta}}
\end{aligned}$$
...
正弦光栅的透过率函数为：
$$T(x)=\frac{1}{2}+\frac{1}{2}\cos{\frac{2\pi x}{d}}=\frac12+\frac14\exp{(\frac{2\pi i x}{d})}+\frac14\exp{(-\frac{2\pi i x}{d})}$$
...
&=\frac12\text{sinc}(\alpha)+\frac14\text{sinc}(\alpha-\pi)+\frac14\text{sinc}(\alpha+\pi)
\end{aligned}$$
所以光强分布为：
$$I=I_0(\frac12\text{sinc}(\alpha)+\frac14\text{sinc}(\alpha-\pi)+\frac14\text{sinc}(\alpha+\pi))^2(\frac{\sin{N\beta}}{\sin{\beta}})^2$$
