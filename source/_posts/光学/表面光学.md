...
$$
会出现复数情况。这时候，反射光和折射光的相位分析会比较复杂。考虑全部情况，我们写出：
$$
\begin{align}
...
\arg(\widetilde{r}_{s}) &= \begin{cases} 0,\dfrac{\sin(i_2-i_1)}{\sin(i_2+i_1)}>0,&i<i_c\\2\arctan\left(\frac{\sqrt{\sin^2{i_1}-\sin^2i_c}}{\cos{i_1}}\right),&i_c<i\end{cases} \\
\end{align}
$$
