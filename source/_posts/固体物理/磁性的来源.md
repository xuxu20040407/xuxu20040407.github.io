...
\end{cases}\Rightarrow M=\dfrac{N g_J^2 \mu_B^2 J(J+1)}{3k_B(T-T_C)}B$$
...
实验事实表明，铂、猛、铬等3d过渡金属元素或稀土元素的磁化率在温度$T<T_N$的时候，磁化率随温度的上升而增大；当温度$T>T_N$的时候，磁化率随温度的上升而减小，且满足奈耳定律：
$$\chi=\dfrac{C}{T+\theta}$$
...
$$B_{A}=B-(\beta M_B+\alpha M_A)$$
$$B_{B}=B-(\beta M_A+\alpha M_B)$$
其中$\beta$表示最近邻的反铁磁交换作用，$\alpha$表示次近邻的反铁磁交换作用，则有$\beta>|\alpha|>0>\alpha$。

套用外斯分子场理论的方程组：
$$\begin{cases}
M_A=N_Ag_J\mu_BJB_J\left(x_A\right)\\
x_A=\dfrac{g_J\mu_BJ}{k_BT}(B-\beta M_B-\alpha M_A)
\end{cases},\quad \begin{cases}
M_B=N_Bg_J\mu_BJB_J\left(x_B\right)\\
x_B=\dfrac{g_J\mu_BJ}{k_BT}(B-\beta M_A-\alpha M_B)
\end{cases}$$

### 奈耳定律
我们讨论外场为0的情况，使用高温近似线性化布里渊函数：
$$\begin{cases}
M_A=-N_Ag_J\mu_BJ\dfrac{J+1}{3J}\dfrac{g_J\mu_BJ}{k_BT}(\beta M_B+\alpha M_A)\\
M_B=-N_Bg_J\mu_BJ\dfrac{J+1}{3J}\dfrac{g_J\mu_BJ}{k_BT}(\beta M_A+\alpha M_B)
\end{cases}$$
设$N_A=N_B$，记：
$$C=\dfrac{Ng_J^2\mu_B^2J(J+1)}{3k_B}=\dfrac{N\mu_J^2}{3k_B}$$
（这个系数我们在铁磁性的结果中也曾看到过：$T_c=C\gamma$）该方程有非零解的临界条件是行列式为0：
$$\begin{vmatrix}
1+\dfrac{C\alpha}{T_N} & -\dfrac{C\beta}{T_N}\\
-\dfrac{C\beta}{T_N} & 1+\dfrac{C\alpha}{T_N}
\end{vmatrix}=0$$
解得奈耳温度：
$$T_N=C(\beta -\alpha)$$
...
$$M=M_A+M_B=C\left[2B-(\alpha+\beta)(M_A+M_B)\right]/T=C\left[2B-(\alpha+\beta)M\right]/T$$
解得：
$$M=\dfrac{2C}{T+C(\alpha+\beta)}B\Rightarrow \chi=\frac{\mu_0 M}{B}=\frac{2C\mu_0}{T+C(\alpha+\beta)}$$
这里的临界温度为：
$$\theta=C(\alpha+\beta)>0$$
...
![image.png](/img/固体物理/尖晶石结构.png)

![image.png](/img/固体物理/尖晶石透视图.png)

假设有$\lambda$份的$X^{2+}$占据了四面体位置，有$\mu$份的$X^{2+}$占据了八面体位置，则化学通式可以写为：
$$(X^{2+}_{\lambda}Y^{3+}_{1-\lambda})[X^{2+}_{1-\mu} Y^{3+}_{1+\mu}]O_4^{2-}$$
其中$\lambda+\mu=1$。当$\lambda=1,\mu=0$时，称为正常尖晶石结构；当$\lambda=0,\mu=1$时，称为反尖晶石结构。

根据平均场理论，AB结构离子的平均场分别为：
$$B_A=\gamma_{AA}M_A-\gamma_{AB}M_B=\gamma_{AA}\lambda M_a-\gamma_{AB}\mu M_b$$
$$B_B=\gamma_{BA}M_A-\gamma_{BB}M_B=\gamma_{BA}\lambda M_a-\gamma_{BB}\mu M_b$$
设$\gamma_{AB}=\gamma_{BA}=\gamma$，$a=\dfrac{\gamma_{AA}}{\gamma},\quad b=\dfrac{\gamma_{BB}}{\gamma}$，则有：

$$\begin{cases}
M_a=C\gamma(B/\gamma+a\lambda M_a-\mu M_b)/T\\
M_b=C\gamma(B/\gamma-\lambda M_a+b\mu M_b)/T
\end{cases}$$

使用类似的方法，不难得到：
- 临界温度：解行列式为0：
  $$\begin{vmatrix}
    1-\dfrac{C\gamma a\lambda}{T_c} & +\dfrac{C\gamma \mu}{T_c}\\
    +\dfrac{C\gamma \lambda}{T_c} & 1-\dfrac{C\gamma b\mu}{T_c}
    \end{vmatrix}=0$$
    解得：
    $$T_c = C\gamma\dfrac{(a\lambda +b\mu)+\sqrt{(a\lambda -b\mu)^2+4\lambda\mu}}{2}$$
...
- 居里-外斯定律：先求解上述方程组的解：
  $$\begin{cases}
    ...
    \dfrac{M_a}{B}=\dfrac{\frac TC- \gamma\mu(b+1)}{(\frac TC-\gamma\mu b)(\frac TC-\gamma\lambda a)-\gamma^2\lambda\mu}\\
    \dfrac{M_b}{B}=\dfrac{\frac TC- \gamma\lambda(a+1)}{(\frac TC-\gamma\mu b)(\frac TC-\gamma\lambda a)-\gamma^2\lambda\mu}
    \end{cases}$$
    磁化率为：
    $$\chi=\frac{\mu_0 M}{B}=\frac{\mu_0 (\lambda M_a+\mu M_b)}{B}= \dfrac{\frac TC- \gamma\lambda\mu(a+b+2)}{(\frac TC-\gamma\mu b)(\frac TC-\gamma\lambda a)-\gamma^2\lambda\mu}$$
    或者表示为：
    $$\frac1\chi= \frac{C(\frac TC-\gamma\mu b)(\frac TC-\gamma\lambda a)-C\gamma^2\lambda\mu}{T- \theta}=\frac{T}{C}+ \frac{\rho}{T- \theta}+\frac{1}{\chi_0}$$
    其中：
    $$\begin{cases}
    \theta=C\gamma\lambda\mu(a+b+2)\\
    \rho=C\gamma^2\lambda\mu[\lambda(a+1)-\mu(b+1)]^2\\
    \chi_0^{-1}=\gamma(2\lambda\mu-\lambda^2 a-\mu^2 b)
    \end{cases}$$
...
| 磁性类型 | 磁化率行为 | 临界温度 | 备注 |
| --- | --- | --- | --- |
| 顺磁性 | 遵循居里定律 $\chi=\dfrac{C}{T}$ | 无 | 高温下的普遍行为 |
...
| 亚铁磁性 | 遵循修正的居里-外斯定律 $\dfrac1\chi= \frac{T}{C}+ \frac{\rho}{T- \theta}+\frac{1}{\chi_0}$ | $T_c = C\gamma\dfrac{(a\lambda +b\mu)+\sqrt{(a\lambda -b\mu)^2+4\lambda\mu}}{2}$<br>$\theta=C\gamma\lambda\mu(a+b+2)$ | 子格磁矩不完全反平行 |
