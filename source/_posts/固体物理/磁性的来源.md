...
- 第三规则：给定自旋$S$和轨道角动量$L$的情况下，如果未填满半壳层，总角动量$J=|L-S|$；如果填满超过半壳层，总角动量$J=|L+S|$。
...
M=Ng_J\mu_B \dfrac{J(J+1)}{3J}x\\
x=\dfrac{g_J\mu_BJ(B+\gamma M)}{k_BT}
...
> 外斯分子场理论仅是唯像理论，由于篇幅所限，我把Heisenberg模型和自旋波理论放在后续章节中讨论 {% post_link '固体物理/自旋波' %} 。
