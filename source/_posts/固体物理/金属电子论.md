...
该基态的总能量为：
$$E_{tot}=\int_0^{m_F} E_F(m_F)\times 2\times\frac18(4\pi m_F^2)dm_F =\frac15 \frac{\pi^{3}\hbar^{2}}{2mL^{2}} m_F^5=\frac{3}{5}E_FN$$
...
> 其中$N(E)=\frac{L^3}{2\pi^2}(2m/\hbar^2)^{3/2}E^{1/2}$（曾在{% post_link '热力学与统计物理/近独立粒子的最概然分布' %}中自由粒子的微观状态描述中得到了证明）。最终得到：
> $$N=\frac{L^3(2m/\hbar^2)^{3/2}}{3\pi^2}E_F^{3/2}$$
> 总的能量为：
> $$E_{tot}=\int_0^{E_F}E N(E)dE=\frac{L^3(2m/\hbar^2)^{3/2}}{5\pi^2}E_F^{5/2}=\frac35E_FN$$
> 这种方法更为简洁。
...
其中$N(E)=\frac{L^3}{2\pi^2}(2m/\hbar^2)^{3/2}E^{1/2}$。

引入无量纲量：$x=\frac{E}{k_BT},\eta=\frac{\mu}{k_BT}$，则有：
$$N=\frac{L^3(2m/\hbar^2)^{3/2}}{2\pi^2}(k_BT)^{3/2}\int_0^{\infty}\frac{x^{1/2}}{e^{x-\eta}+1}dx$$
从中发现了费米狄拉克积分：
$$F_{1/2}(\eta)=\int_0^{\infty}\frac{x^{1/2}}{e^{x-\eta}+1}dx$$
在低温极限$\eta\gg 1$下，费米狄拉克积分近似为：
$$F_{1/2}(\eta)\approx \frac23\eta^{3/2}(1+\frac{\pi^2}{8}\eta^{-2}+\cdots)$$
所以：
$$N=\frac{L^3(2m/\hbar^2)^{3/2}}{3\pi^2}\mu(T)^{3/2}(1+\frac{\pi^2}{8}\eta_F^{-2})$$
反解得到：
$$\begin{aligned}
...
### 比热容
对能量积分：
$$E_{tot}=\int_0^{\infty}E N(E)f(E)dE=\frac{L^3(2m/\hbar^2)^{3/2}}{2\pi^2}(k_BT)^{5/2}\int_0^{\infty}\frac{x^{3/2}}{e^{x-\eta}+1}dx$$
使用费米狄拉克积分展开：
$$F_{3/2}(\eta)\approx \frac{2}{5}\eta^{5/2}(1+\frac{5\pi^2}{8}\eta^{-2}+\cdots)$$
得到：
$$\begin{aligned}E_{tot}&=\frac25\frac{L^3(2m/\hbar^2)^{3/2}}{2\pi^2}\mu^{5/2}(T)(1+\frac{5\pi^2}{8}\eta_F^{-2})\\
&=\frac35 N\mu(0)^{-3/2}\mu(T)^{5/2}(1+\frac{5\pi^2}{8}\eta_F^{-2})\\
&=E_{tot}(0)\frac{\mu^{5/2}(T)}{\mu^{5/2}(0)}(1+\frac{5\pi^2}{8}\eta_F^{-2})\\
&=E_{tot}(0)\left[1 - \frac{\pi^2}{12}(\frac{T}{T_F})^2\right]^{5/2}\left[1+\frac{5\pi^2}{8}(\frac{T}{T_F})^{2}\right]\\
