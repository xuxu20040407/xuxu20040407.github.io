...
$$f(x)=\frac{a_0}2+\sum_{n=1}^\infty \left[a_n\cos\left(\dfrac{\pi nx}{L}\right)+b_n\sin\left(\dfrac{\pi nx}{L}\right)\right]$$
其中，考虑到归一化系数：
$$\int_{-L}^L 1dx=2L,\int_{-L}^L \cos^2\left(\dfrac{\pi nx}{L}\right)dx=L,\int_{-L}^L \sin^2\left(\dfrac{\pi nx}{L}\right)dx=L$$
可以得到傅里叶展开的系数为：
...
a_n=\dfrac{1}{L}\int_{-L}^L f(x)\cos\left(\dfrac{\pi nx}{L}\right)dx\\
b_n=\dfrac{1}{L}\int_{-L}^L f(x)\sin\left(\dfrac{\pi nx}{L}\right)dx
\end{cases}$$
...
系数为：
$$c_n=\dfrac{1}{2L}\int_{-L}^L f(x)e^{-i\frac{\pi nx}{L}}dx$$
...
- 非正则Sturm-Liouville问题：定义在无限区间$(-\infty,+\infty)$上，或不满足任一正则条件。
