...
$$u(\vec{r})=\int_S \left[u(\vec{r_0})\frac{\partial G(\vec{r},\vec{r_0})}{\partial n}-G(\vec{r},\vec{r_0})\frac{\partial u(\vec{r_0})}{\partial n}\right]dS$$
对于一般的Robin边界条件：
$$
...
$$\begin{aligned}
\int_S (u\frac{\partial G}{\partial n}-G\frac{\partial u}{\partial n})dS&=\int_V [u\nabla^2 G- G\nabla^2 u]dV\\
\Rightarrow c^2\int_t\int_S (u\frac{\partial G}{\partial n}-G\frac{\partial u}{\partial n})dSdt&=\int_t\int_V [uG_{tt}+u\delta(\vec{r}-\vec{r_0})\delta(t-t_0)- Gu_{tt}-Gf]dVdt\\
\end{aligned}$$
整理后可以得到：
...
$$u(\vec{r}_0)=\int_t\int_V G f(\vec{r}) dVdt+c^2\int_t\int_S (u\frac{\partial G}{\partial n}-G\frac{\partial u}{\partial n})dSdt-\int_V \left[uG_t -Gu_t\right]\Big|\,dV$$
交换$\vec{r}$和$\vec{r_0}$，可以得到：
$$\begin{aligned}
