...
- $\frac{dy}{dx}=-\frac{M(x,y)}{N(x,y)}$（全微分方程）
  ...
  > 为了方便起见，通常选择积分因子只依赖于一个变量：若$\mu_y=0$，则$\mu=\exp\left(\int\frac{M_y-N_x}{N}dx\right)$；若$\mu_x=0$，则$\mu=\exp\left(\int\frac{N_x-M_y}{M}dy\right)$。这样就可以将方程化为全微分方程。
...
- 对于$m\geq 1$重复共轭复根$\mu\pm i\nu$，在通解中加入如下的线性组合：
  $$(C_0\exp(\mu x)+C_1x\exp(\mu x)+\cdots+C_{m-1}x^{m-1}\exp(\mu x))\cos(\nu x)+\\(d_0\exp(\mu x)+d_1x\exp(\mu x)+\cdots+d_{m-1}x^{m-1}\exp(\mu x))\sin(\nu x)$$
...
> 将特解带入方程，并利用$y_1(x),y_2(x)$满足齐次方程$y''+py'+qy=0$消去相应项，可得：
> $$y_1'(x)z_1'(x)+y_2'(x)z_2'(x)=f(x)$$
> 现在我们得到了两个方程：
> $$\begin{cases}
...
> 可以解得：
> $$\begin{cases}
> z_1'(x)=-\dfrac{y_2(x)f(x)}{W(y_1,y_2)}\\
> z_2'(x)=\dfrac{y_1(x)f(x)}{W(y_1,y_2)}\\
> \end{cases}$$
