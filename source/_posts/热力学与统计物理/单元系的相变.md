...
tags: 统计物理
categories: 统计物理
cover: "https://upload.wikimedia.org/wikipedia/commons/f/fe/Phase-diag-Xiang-zh-hans.svg"
---
...
&=[\frac{\partial}{\partial U}(\frac{1}{T})\delta U+\frac{\partial}{\partial V}(\frac{1}{T})\delta V]\delta U+[\frac{\partial}{\partial U}(\frac{p}{T})\delta U+\frac{\partial}{\partial V}(\frac{p}{T})\delta V]\delta V\\
...
当$T\leq T_c$的时候，该区域由于$(\dfrac{\partial p}{\partial V_m})_T>0$，不稳定，气体会直接越过气液两相共存的阶段直接进入液相。
...
$$C=-T\frac{\partial^2 F}{\partial T^2}=\begin{cases}-T\frac{\partial^2 F_0}{\partial T^2}&T\geq T_c\\-T\frac{\partial^2 F_0}{\partial T^2}+T\frac{\alpha_0^2}{2\beta_0 T_c^2}&T\leq T_c\end{cases}$$
可以看到，比热在临界点处有突变，大小为$\frac{\alpha_0^2}{2\beta_0 T_c}$。
