...
1. **理想气体状态方程**：$pV=nRT$
2. **理范德瓦尔斯方程**：$(p+\frac{an^2}{V^2})(V-nb)=nRT$
...
|:--------:| :---------:|:--------:|:--------:|:--------:|
| 多方过程 | $pV^{n}=const$ <br>$TV^{n-1}=const$<br>$\dfrac{p^{n-1}}{T^{n}}=const$ |$\int_{V_1}^{V_2}pdV=\dfrac{p_1V_1}{n-1}$<br>$[(\dfrac{V_1}{V_2})^{n-1}-1]$|$C_m\Delta T$|$C_m=\dfrac{n-\gamma}{n-1}C_{V,m}$|
