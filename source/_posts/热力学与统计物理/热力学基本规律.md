...
**物态方程**：对于一个系统，描述参量和平衡状态的函数称为物态方程：
$$f(p,V,T)=0$$
其中：
...
**内能和焓**：理想气体的内能与体积无关，因而可以写成：
$$U=C_VT$$
同理，焓也只是温度的函数，那么：
$$H=C_PT$$

**准静态过程和绝热过程**：气体以极缓慢的速度历经一系列过程，可视作一直处于平衡态。如果气体处于绝热过程：
...
**热机**：热机是指利用内能做功的机械。在准静态过程的语境下，热机的过程可以画在$p-V$图上。
...
可以用对应的热力学基本公式辅助记忆：
$$dA=Bdx+Cdy\Rightarrow (\frac{\partial B}{\partial y})_{x}=(\frac{\partial C}{\partial x})_{y}$$
...
那么
$$C_P-C_V=T(\frac{\partial S}{\partial T})_{P}-T(\frac{\partial S}{\partial T})_{V}=T(\frac{\partial p}{\partial T})_{V}(\frac{\partial V}{\partial T})_{P}$$

其中
$$\begin{aligned}
T(\frac{\partial S}{\partial T})_{P}-T(\frac{\partial S}{\partial T})_{V}
&=T[\frac{\partial{(S,P)}}{\partial{(T,P)}}-\frac{\partial{(S,V)}}{\partial{(T,V)}}]\\
&=T[\frac{\partial{(S,P)}}{\partial{(T,P)}}-\frac{\partial{(S,V)}}{\partial{(T,V)}}]\\
