...
$$\begin{aligned}
f(p_x,p_y,p_z)dp_xdp_ydp_z&=\frac{dp_xdp_ydp_z}{h^3}e^{-\alpha-\frac{\beta}{2m}\sum_ip_i^2}\\&=\frac{N}{V}(\frac{h^2}{2\pi mkT})^\frac32\frac{dp_xdp_ydp_z}{h^3}e^{-\frac{\beta}{2m}\sum_ip_i^2}\\&=n(\frac{1}{2\pi mkT})^\frac32e^{-\frac{1}{2mkT}\sum_ip_i^2}dp_xdp_ydp_z\end{aligned}$$
如果是速度为变量：
...
- 平均速度：
$$\begin{aligned}
\bar{v}&=\frac{1}{n}\int v f(v)dv\\
&=4\pi n(\frac{m}{2\pi kT})^\frac32\int v^3e^{-\frac{mv^2}{2kT}}dv\\
&=4\pi n(\frac{m}{2\pi kT})^\frac32 2(\frac{kT}{m})^2\\
&=\sqrt{\frac{8kT}{\pi m}}
\end{aligned}$$

- 方均根速率：
$$\begin{aligned}
\bar{v^2}&=\frac{1}{n}\int v^2 f(v)dv\\
&=4\pi n(\frac{m}{2\pi kT})^\frac32\int v^4e^{-\frac{mv^2}{2kT}}dv\\
&=4\pi n(\frac{m}{2\pi kT})^\frac32\frac{3\sqrt{\pi}}{8}(\frac{2kT}{m})^\frac52\\
&=\frac{3kT}{m}
\end{aligned}$$
- 速度方差：
$$\begin{aligned}
\overline{\Delta v^2}&=\bar{v^2}-\bar{v}^2\\
&=\frac{3kT}{m}-\frac{8kT}{\pi m}\\
&=\frac{kT}{m}(3-\frac{8}{\pi})
\end{aligned}$$
- 最概然速率：
$$\begin{aligned}
...
## 双原子气体的内能和热容
能均分定理适合用于经典范围，我们现在用量子理论讨论理想双原子分子气体的内能和热容。
...
振动能的配分函数为：
$$Z^v_1=\sum_{n=0}^\infty e^{-\beta\hbar\omega(n+\frac12)}=e^{-\frac{\beta\hbar\omega}{2}}\sum_{n=0}^\infty e^{-\beta\hbar\omega n}=\frac{e^{-\frac{\beta\hbar\omega}{2}}}{1-e^{-\beta\hbar\omega}}$$
内能和热容为：
$$\begin{cases}U^v=-N\frac{\partial}{\partial\beta}\ln Z^v_1=\frac{N\hbar\omega}{2}+\frac{N\hbar\omega}{e^{\beta\hbar\omega}-1}\\
C^v=\frac{\partial U^v}{\partial T}=\frac{N(\hbar\omega)^2}{kT^2}\frac{e^{\beta\hbar\omega}}{(e^{\beta\hbar\omega}-1)^2}\end{cases}$$
...
| 量子 | $\frac32NkT$ | $\frac{Nk\theta_v}{2}+\frac{Nk\theta_v}{e^{\theta_v/T}-1}$ | $NkT$ | $\frac52NkT+\frac{Nk\theta_v}{2}+\frac{Nk\theta_v}{e^{\theta_v/T}-1}$ |
...
总的固体的内能为：
$$U=-3N\frac{\partial}{\partial\beta}\ln Z_1=\sum_{i=1}^{3N}(\frac{\hbar\omega_i}{2}+\frac{\hbar\omega_i}{e^{\beta\hbar\omega_i}-1})=3N(\frac{\hbar\omega}{2}+\frac{\hbar\omega}{e^{\beta\hbar\omega}-1})$$
...
在弱场和高温下，可以近似为：
$$M=n\mu\beta\mu B=n\mu^2\beta B\simeq n\mu^2\beta\mu_0H$$

这就是我们熟知的居里定律。其中$\chi=\dfrac{\mu_0 M}{B}=n\mu^2\beta\mu_0$。

在强场和低温($\frac{\mu B}{kT}\gg 1$)下，可以近似为：
$$M=n\mu$$
