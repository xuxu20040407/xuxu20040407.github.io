...
> Note：对于玻色气体，如果温度太低，$g_{\frac32}(e^{-\alpha})$会达到其上界$g_{\frac32}(1)=\zeta(\frac32)\approx2.612$，即$e^{-\alpha}\to1$，此时玻色-爱因斯坦凝聚会发生，这时候上述式子就会因为$D(\epsilon=0)=0$而不成立。这一点在下一节会讨论。
...
(z=1)&= \frac32 N_{\text{exc}}kT\frac{g_{\frac52}(z)}{g_{\frac32}(z)}\\
...
1.925Nk\left(\frac{T}{T_c}\right)^{3/2} & T<T_c
\end{cases}$$
化学势为：
$$\mu = \begin{cases}
0 & T<T_c\\
...
> 得到内能和压强：
> $$\begin{cases}U(T) &= -\frac{\partial Z_1}{\partial \beta} = \frac{3V}{2\pi^2c^3}\frac{2}{\beta^4\hbar^3}\\P(T) &= \frac{1}{\beta}\frac{\partial Z_1}{\partial V} = \frac{1}{\pi^2c^3}\frac{1}{\beta^4\hbar^3}\end{cases}\Rightarrow U=3PV$$
> 对比理想气体的内能和压强：
...
内能密度的最大值有：
$$\frac{d}{dx}\frac{x^3}{e^x-1}=0,x=\beta\hbar\omega\Rightarrow \hbar\omega=2.822kT$$
这就是维恩位移定律。

> 也可以使用巨配分函数来计算，效果是一样的。光子气体的巨配分函数为：
> $$\Xi = \prod_l (1-e^{-\beta\epsilon_l})^{-\omega_l} = \prod_l (1-e^{-\beta\hbar\omega})^{-2}$$
> $$\ln \Xi = -\frac{V}{\pi^2c^3}\int_0^\infty \omega^2\ln(1-e^{-\beta\hbar\omega})d\omega=\frac{\pi^2V}{45c^3}\frac{1}{(\beta\hbar)^3}$$
> 所以内能：
> $$U = -\frac{\partial \ln \Xi}{\partial \beta} = \frac{\pi^2V}{15c^3}\frac{1}{\beta^4\hbar^3}$$
> 
> 压强：
...
电子的量子态数为：
$$D(\epsilon)d\epsilon = \frac{V}{2\pi^2}(\frac{2m}{\hbar^2})^{3/2}\epsilon^{1/2}d\epsilon$$
平均电子数为：
$$dn = \frac{V}{2\pi^2}(\frac{2m}{\hbar^2})^{3/2}\frac{\epsilon^{1/2}}{e^{\beta(\epsilon-\mu)}+1}d\epsilon$$

化学势通过粒子总数求得：
$$N = \int_0^\infty \frac{V}{2\pi^2}(\frac{2m}{\hbar^2})^{3/2}\frac{\epsilon^{1/2}}{e^{\beta(\epsilon-\mu)}+1}d\epsilon$$

对于0K的电子气：

$$\begin{cases}a_l=2,\epsilon<\mu(0)\\a_l=0,\epsilon>\mu(0)\end{cases}$$
所以：
$$N = \int_0^{\mu(0)} \frac{V}{2\pi^2}(\frac{2m}{\hbar^2})^{3/2}\epsilon^{1/2}d\epsilon$$
解得：
