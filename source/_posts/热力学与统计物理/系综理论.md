...
斯特林近似后：
$$S=Nk\{\ln[\frac{V}{N}(\frac{4\pi mE}{3Nh^2})^{3/2}]+\frac52\}+k(\ln\frac{3N}{2}+\ln\frac{dE}{E})$$
后两项在热力学极限下可以忽略：
$$S=Nk\{\ln[\frac{V}{N}(\frac{4\pi mE}{3Nh^2})^{3/2}]+\frac52\}$$
...
> 解题指南：写出各量子态的能量$E_s$，求出量子或经典情况的表达式$E(q,p)$，进而求得配分函数的表达式。正则系综的特性函数为$F=-kT\ln Z(N,V,T)$，利用自由能这一特性函数求出其他热力学量：
> $$dF=-SdT-pdV+\mu dN$$
> 所以：
...
> 所以相对涨落为：
> $$\frac{\bar{E^2}-\bar{E}^2}{\bar{E}^2}=\frac{kT^2C_V}{\bar{E}^2}\propto \frac1N$$
...
算出的熵为：
$$S\propto \frac{3N-2}{2}\ln E$$
对他们分别求导，得到：
$$E=\frac{\partial F}{\partial T^{-1}}=\frac{3N}{2}T,\quad \frac1T=\frac{\partial S}{\partial E}=\frac{3N-2}{2}\frac{1}{E}$$
...
气体的压强为：
$$\begin{aligned}p&=-\frac{\partial}{\partial V}(-kT\ln Z)\\&=kT\frac{\partial}{\partial V}(N\ln V+\frac{N^2}{2V}\int f_{12}d\vec{r}_{12})\\&=\frac{NkT}{V}(1-\frac{N}{2V}\int f_{12}d\vec{r}_{12})\end{aligned}$$
即：
...
当$T\ll \frac{\hbar\omega_D}{k}$时，可以近似为：
$$U-U_0\propto T^3$$
...
同样的方法，可以得到
$$\begin{aligned}\ln \Omega(E_0-E_s,N_0-N)&=\ln \Omega(E_0,N_0)-\frac{\partial \ln\Omega}{\partial E}E_s-\frac{\partial \ln\Omega}{\partial N}N\\&=\ln \Omega(E_0)-\beta{E_s}-\alpha N\end{aligned}$$
...
||微正则系综|正则系综|巨正则系综|
|---|---|---|---|
|系统|孤立系统（无粒子能量交换）|闭合系统（有能量交换）|开放系统（有粒子能量交换）|
