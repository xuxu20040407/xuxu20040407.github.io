...
- 广义动量守恒条件：
  $$\boxed{\frac{\partial L}{\partial q_j}=0\Rightarrow p_j=Const}$$
...
&=\frac{\partial }{\partial t}\left(\frac{\partial \vec{r}_i}{\partial q_\beta}\right) + \sum_{\alpha=1}^{N-k}\dot{q}_\alpha\frac{\partial}{\partial q_\alpha}\left(\frac{\partial \vec{r}_i}{\partial q_\beta}\right)\\
...
抛物面$z(r)=Ar^2$上，小球的拉格朗日量在柱面坐标系下可以写为：
$$\begin{aligned}
T &= \frac12 m \left[\dot{r}^2 + r^2\dot{\theta}^2 + (2Ar\dot{r})^2\right] =  \frac12 m \left[(1+4A^2r^2)\dot{r}^2 + r^2\dot{\theta}^2\right]\\
V &= mgz = mgAr^2
\end{aligned}$$
$$\boxed{L = \frac12m(1+4A^2r^2)\dot{r}^2 + \frac12mr^2\dot{\theta}^2 - mgAr^2}$$

$L$不显含$\theta$，故角动量守恒：
$$\frac{\partial L}{\partial \dot{\theta}} = mr^2\dot{\theta} = l = \text{const}$$
利用守恒量消去$\dot{\theta}$，能量写作：
$$E = \frac12m(1+4A^2r^2)\dot{r}^2 + \frac{l^2}{2mr^2} + mgAr^2$$
即径向运动由**有效势**决定：
$$\boxed{V_{eff}(r) = \frac{l^2}{2mr^2} + mgAr^2}$$

**圆轨道**满足$V_{eff}'(r_0)=0$：
$$-\frac{l^2}{mr_0^3} + 2mgAr_0 = 0\Rightarrow \dot{\theta}^2 = \frac{l^2}{m^2r_0^4} = 2gA$$
$$\boxed{\dot{\theta} = \sqrt{2gA},\qquad r_0 = \left(\frac{l^2}{m^2\cdot 2gA}\right)^{\frac14}}$$

**稳定性**：圆轨道附近
$$V_{eff}''(r_0) = \frac{3l^2}{mr_0^4} + 2mgA = 3(2mgA) + 2mgA = 8mgA > 0$$
$V_{eff}$在$r_0$处恒为严格极小值，**任意半径的圆轨道均稳定**。

**径向小振动**：径向拉格朗日方程为
$$(1+4A^2r^2)\ddot{r} + 4A^2r\dot{r}^2 - r\dot{\theta}^2 + 2gAr = 0$$
令$\xi = r - r_0$并线性化，得$\ddot{\xi} + \omega_r^2\xi = 0$：
$$\boxed{\omega_r^2 = \frac{8gA}{1+4A^2r_0^2}}$$

> **频率比**：$\dfrac{\omega_r}{\dot{\theta}} = \dfrac{2}{\sqrt{1+4A^2r_0^2}}$一般不是有理数，径向与角向不同频，轨道一般不闭合；仅当$4A^2r_0^2$使该比值为有理数时闭合。$r_0\to 0$时比值为$2$，回到二维谐振子的椭圆轨道。

### 推广：$z = Ar^n$

将抛物面推广为一般幂次曲面$z=Ar^n$。此时$\dot{z}=nAr^{n-1}\dot{r}$，故：
$$\begin{aligned}
v^2 &= (1+n^2A^2r^{2n-2})\dot{r}^2 + r^2\dot{\theta}^2\\
L &= \frac12m(1+n^2A^2r^{2n-2})\dot{r}^2 + \frac12mr^2\dot{\theta}^2 - mgAr^n
\end{aligned}$$
角动量$l=mr^2\dot{\theta}$仍守恒，有效势：
$$V_{eff}(r) = \frac{l^2}{2mr^2} + mgAr^n$$

**圆轨道**：$V_{eff}'(r_0)=0$给出
$$-\frac{l^2}{mr_0^3} + mgAnr_0^{n-1} = 0\Rightarrow \frac{l^2}{m^2r_0^4} = gAnr_0^{n-2}$$
$$\boxed{\dot{\theta}^2 = gAn\,r_0^{\,n-2},\qquad r_0 = \left(\frac{l^2}{m^2gAn}\right)^{\frac{1}{n+2}}}$$

> 与$n=2$不同，一般$n\neq 2$时角速度按幂次$\dfrac{n-2}{2}$随半径变化：$n>2$时外圈转得更快，$n<2$时内圈更快。给定角动量后$r_0$被**唯一**确定。

**稳定性**：
$$V_{eff}''(r_0) = \frac{3l^2}{mr_0^4} + mgAn(n-1)r_0^{n-2} = mgAn(n+2)r_0^{n-2}$$
当$A>0,\ n>0$时恒为正，**所有$n>0$的圆轨道均稳定**；$n<0$时$V_{eff}'=0$无正根，**不存在圆轨道**。

**径向小振动**：径向方程在$r_0$处线性化，得：
$$\boxed{\omega_r^2 = \frac{gAn(n+2)r_0^{\,n-2}}{1+n^2A^2r_0^{\,2n-2}},\qquad \frac{\omega_r^2}{\dot{\theta}^2} = \frac{n+2}{1+n^2A^2r_0^{\,2n-2}}}$$

> **受力视角的验算**：曲面法线的水平分量与竖直分量之比等于斜率$\dfrac{N_r}{N_z}=\tan\alpha = nAr^{n-1}$。圆轨道上$N_z=mg$、$N_r=mr\dot{\theta}^2$，立即得$\dot{\theta}^2=gAnr^{n-2}$，与拉格朗日结果一致。可见约束反力随半径**自适应**地提供向心力分量——这正是曲面约束与有心力场的本质区别：中心吸引势$V\propto -r^{-n}$（见"有心力和散射问题"）的圆轨道稳定性要求$n<2$，而曲面约束下只需$n>0$。
