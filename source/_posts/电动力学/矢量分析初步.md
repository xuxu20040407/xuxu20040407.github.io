...
>
>$\mathbf{A}\times\mathbf{B}=\epsilon_{ijk}A_iB_j\vec{e_k}\left.=\left|\begin{array}{ccc} {A_{x}} & {A_{y}}&{A_{z}}\\{B_{x}}&{B_{y}}&{B_{z}}\\{e_{x}}&{e_{y}}&{e_{z}}\end{array}\right.\right|$
>
...
2. 矢量散度的梯度：
$$\nabla(\nabla\cdot\mathbf{A})=\partial_i\partial_j A_j \vec{e_i}$$
3. 标量梯度的旋度：
$$\nabla\times(\nabla \varphi)=0.$$
4. 矢量旋度的散度：
