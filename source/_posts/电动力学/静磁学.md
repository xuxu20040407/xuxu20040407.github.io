...
静磁学的核心是恒定电流，尽管电流是非静止的，但是由其产生的磁场是静止的。由于没有磁荷磁场是无源的，电流是静磁学中磁场唯一的旋度贡献。注意，恒定电流的散度为零；毕奥-萨伐尔定律推导中的旋度算符作用于场点坐标，故$\nabla\times\vec{J}=0$：
$$\nabla\cdot \vec{J}=0,\nabla\times \vec{J}=0$$
...
其中，使用了：
$$(\vec{J}\cdot\nabla)\frac{\hat{r}}{r^2}=\sum_i\nabla\cdot(\frac{r_i}{r^3}\vec{J})-\frac{\hat{r}}{r^2}\nabla\cdot\vec{J}$$
...
| 有限长直导线 | $\vec{B}=\dfrac{\mu_0 I}{4\pi r}(\cos\theta_2+\cos\theta_1) \hat{\phi}$ | None |
