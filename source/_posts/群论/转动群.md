...
$$\begin{aligned}
\mathbf{R} &= \mathbf{R}_{z}(\alpha) \mathbf{R}_{y'}(\beta) \mathbf{R}_{z''}(\gamma) \\
&=\begin{pmatrix}
...
$$\begin{aligned}
\mathbf{U} &= \mathbf{U}_{z}(\alpha) \mathbf{U}_{y'}(\beta) \mathbf{U}_{z''}(\gamma)\\
&=\begin{pmatrix}
e^{-i\alpha/2} & 0 \\
0 & e^{i\alpha/2}
\end{pmatrix}\begin{pmatrix}
...
先计算三种最简单的情况，分别对应只有$\alpha$转动、只有$\beta$转动、只有$\gamma$转动。

1. 只有$\alpha$转动时（只有$\gamma$转动同理）：
