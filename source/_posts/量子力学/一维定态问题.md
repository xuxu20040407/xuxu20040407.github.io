...
3. **参数**：
- $\langle x\rangle=0$
- $\langle x^2\rangle=\dfrac{2n+1}{2m\omega}\hbar$
- $\langle p\rangle=0$
- $\langle p^2\rangle=\dfrac{2n+1}{2}m\omega\hbar$
...
2. **透射率**：$T=\dfrac{1}{1+\left(\dfrac{m\alpha}{\hbar^2k_1}\right)^2}$
3. **反射率**：$R=\dfrac{1}{1+\left(\dfrac{\hbar^2k_1}{m\alpha}\right)^2}$
...
只有当$\dfrac{\hbar^2}{m\alpha}<2L$的时候才开始有解。
...
根据Bloch定理，$\psi(x+L)=e^{iqL}\psi(x)$，设
$$\psi(x)=Ae^{ikx}+Be^{-ikx}$$
那么
...
$$\cos{kL}-\frac{m\alpha}{\hbar^2k}\sin{kL}=\cos{qL}$$
...
$$\dfrac{(-V_0-2E)}{2 \sqrt{-E(V_0+E)}}\sinh{\dfrac{b\sqrt{-2mE}}{\hbar} }\sin{\dfrac{(a-b)\sqrt{2m(V_0+E)}}{\hbar}}+\cosh{\dfrac{b\sqrt{-2mE}}{\hbar}}\cos{\dfrac{(a-b)\sqrt{2m(V_0+E)}}{\hbar}}=\cos{qa}$$
