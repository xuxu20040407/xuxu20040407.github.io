...
def psi_a(x):
    return (1/(np.sqrt(2*np.pi)*sigma_a))**(1/2) * np.exp(-x**2/(4*sigma_a**2))

def psi_b(x):
    return (1/(np.sqrt(2*np.pi)*sigma_b))**(1/2) * np.exp(-x**2/(4*sigma_b**2))
...
$$\frac18\times\frac43 \pi n^3\times 2=N\rightarrow n=\sqrt[3]{\frac{3N}{\pi}}$$
