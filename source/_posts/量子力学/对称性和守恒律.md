...
> 从历史地位上来说，Ehrenfest定理是经典力学与量子力学的桥梁。它的形式和哈密顿的正则方程十分类似：
> $$\frac{dA}{dt}=\frac{\partial A}{\partial t}+\{A,H\}$$
> 正如 {% post_link '量子力学/量子力学的诞生' %} 中提到，狄拉克注意到了这种相似性，并进一步提出了正则量子化的思想。
>
> 需要注意的是，虽然和下面提到的海森堡方程很像：
> $$i{\hbar}\partial_t\hat{F}_H=[\hat{F}_H,\hat{H}]$$
> 但这两个方程是两回事：Ehrenfest定理是对力学量平均值的描述，是对$\langle\psi|\hat A|\psi\rangle$求导；而海森堡方程是对算符的描述，是对$\hat F_H=\sum_iF_i\hat{U}^\dagger|\psi_i\rangle\langle\psi_i|\hat{U}=\sum_iF_i|\psi_i(-t)\rangle\langle\psi_i(-t)|,\quad(\hat F_S=\sum_iF_i|\psi_i\rangle\langle\psi_i|)$求导。之所以方程形式一样，是因为：
> $$\dfrac{\dfrac{d}{dt}\langle\psi|}{\langle\psi|}=-\frac{1}{i\hbar}\hat{H}=\dfrac{\dfrac{d}{dt}|\psi(-t)\rangle}{|\psi(-t)\rangle}$$
> （绘景相反导致一个负号，左右矢相反导致一个负号）。这一点在密度算符的演化中得以体现：
> $$\frac{d\hat\rho}{dt}=\frac{\partial\hat\rho}{\partial t}+\frac{1}{i\hbar}[\hat H,\hat \rho]$$
> 密度算符在薛定谔绘景下演化，但是是对右矢求导，所以与Ehrenfest定理差了一个负号。同理，对比海森堡绘景下的算符演化，都是对右矢求导，但是绘景相反，所以相差负号。
...
$$i\hbar\frac{\partial\psi^\prime}{\partial t}=\hat{H}\psi^\prime.$$
可以导出：
...
满足幺正变换条件：
$$\begin{aligned}\hat{Q}^{\dagger}\hat{Q}&=(\hat{I}-i\epsilon\hat{F}^{\dagger})(\hat{I}+i\epsilon\hat{F})\\&=\hat{I}+i\epsilon(\hat{F}-\hat{F}^{\dagger})\\&=\hat{I}\end{aligned}$$
...
这要求$\lambda^2=1\Rightarrow \lambda=\pm1$。对于本征值为1，称为偶宇称；对于本征值为-1，成为奇宇称。
...
**旋转算符**：$\hat{R_z}(a)=\exp(\dfrac{-ia}{\hbar}\hat{L_z})$
...
\left\langle n^{\prime}\ell^{\prime}m^{\prime}\right|\hat{V}_z\left|n\ell m\right\rangle =C_{m0m^{\prime}}^{\ell0\ell^{\prime}}\left\langle n\ell^{\prime}\parallel V\parallel n\ell\right\rangle. 
\end{gathered}$$
...
$$i{\hbar}\partial_t\hat{U}(t)=\hat{H}(t)\hat{U}(t)$$
薛定谔绘景描述了在确定的算符作用下，量子态的演化过程。当哈密顿量不含时，薛定谔方程求解为：
$$\hat{U}(t)=\exp\left[-\frac{i}{\hbar}\hat{H}t\right]$$
在能量表象下，将时间演化算符作用到态矢量上：
$$|\psi(t)\rangle=\hat{U}(t)|\psi(0)\rangle=\sum_n c_n e^{-iE_nt/\hbar} |n\rangle$$

## 海森堡绘景

海森堡绘景描绘了量子态确定的情况下，算符随时间的演化。海森堡方程定义算符的演化为：
$$\hat{F}_H(t)=\hat{U}^\dagger(t)\hat{F}_S\hat{U}(t)$$
对其求导，得到：
$$\begin{aligned}
\frac{d}{dt}\hat{F}_H(t)&=\frac{d}{dt}\left[\hat{U}^\dagger(t)\hat{F}_S\hat{U}(t)\right] \\
&=\frac{d\hat{U}^\dagger(t)}{dt}\hat{F}_S\hat{U}(t)+\hat{U}^\dagger(t)\hat{F}_S\frac{d\hat{U}(t)}{dt}+\frac{\partial}{\partial t}\hat{F}_H \\
&=\left(\frac{1}{i\hbar}\hat{H}\hat{U}\right)^\dagger(t)\hat{F}_S(t)\hat{U}(t)+\hat{U}^\dagger(t)\hat{F}_S\left(\frac{1}{i\hbar}\hat{H}\hat{U}(t)\right)+\frac{\partial}{\partial t}\hat{F}_H \\
&=\frac{i}{\hbar}\left(\hat{U}^\dagger(t)\hat{H}\hat{F}_S(t)\hat{U}(t)-\hat{U}^\dagger(t)\hat{F}_S(t)\hat{H}\hat{U}\right)+\frac{\partial}{\partial t}\hat{F}_H \\
&=\frac{i}{\hbar}\left(\hat{H}\hat{U}^\dagger(t)\hat{F}_S(t)\hat{U}(t)-\hat{U}^\dagger(t)\hat{F}_S(t)\hat{U}\hat{H}\right)+\frac{\partial}{\partial t}\hat{F}_H \\
&=\frac{i}{\hbar}\left(\hat{H}\hat{F}_H(t)-\hat{F}_H(t)\hat{H}\right)+\frac{\partial}{\partial t}\hat{F}_H \\
&=\frac{i}{\hbar}[\hat{H},\hat{F}_H(t)]+\frac{\partial}{\partial t}\hat{F}_H\\
\end{aligned}$$
如果算符$\hat{F}_H$不依赖时间，则有：
$$i{\hbar}\partial_t\hat{F}_H=[\hat{F}_H,\hat{H}]$$
...
其中$\hat{H}_0$是体系自身的哈密顿量，$\hat{H}_I$是相互作用哈密顿量。相互作用绘景认为量子态在$\hat{H}_I(t)$下按照薛定谔绘景演化，而算符（包括$\hat{H}_I$）在$\hat{H}_0$下按照海森堡绘景演化。令：
$$\psi_I(t)=\exp(i\hat{H}_0 t/\hbar)\psi_S(t)$$
其中$\psi_S(t)$是薛定谔绘景下的量子态。对相互作用绘景下的量子态求导：
$$\begin{aligned}
\frac{d}{dt}\psi_I(t)&=\frac{d}{dt}\left[\exp(i\hat{H}_0 t/\hbar)\psi_S(t)\right] \\
&=\frac{1}{i\hbar}\exp(i\hat{H}_0 t/\hbar)\hat{H}\psi_S(t)+\frac{i}{\hbar}\exp(i\hat{H}_0 t/\hbar)\hat{H}_0\psi_S(t) \\
&=\frac{1}{i\hbar}\exp(i\hat{H}_0 t/\hbar)(\hat{H}-\hat{H}_0)\psi_S(t) \\
&=\frac{1}{i\hbar}\exp(i\hat{H}_0 t/\hbar)\hat{H}_I\exp(-i\hat{H}_0 t/\hbar)\psi_I\\
&=\frac{1}{i\hbar}\hat{H}_I(t)\psi_I\\
\end{aligned}$$


可见，相互作用绘景下的量子态演化与$\hat{H}_I(t)=\exp(i\hat{H}_0 t/\hbar)\hat{H}_I\exp(-i\hat{H}_0 t/\hbar)$相关。同样令：
...
&=\frac{i}{\hbar}[\hat{H}_0,\hat{F}_I(t)]+\frac{\partial}{\partial t}\hat{F}_I\\
\end{aligned}$$
