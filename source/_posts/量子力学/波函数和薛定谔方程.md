...
- 根据势能项的形式，求解定态初值解$\psi(\vec{r},0)$。这一点在后续的{% post_link '量子力学/一维定态问题' %}中会详细介绍。
- 利用初值条件，求解通解。我们在下面叙述。
...
做简单的变换：
$$\mathrm{i}\hbar \dot{f}(t)=-\dfrac{\hbar^2}{2m}\psi_{xx}(x)+V(x)$$
...
第二个方程也称作**定态薛定谔方程**，求解空间项。从第一个时间方程出发，可以很容易地写出其演化的函数形式：
