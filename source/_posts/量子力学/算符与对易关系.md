...
如果二者能做内积，就要求$n=m$：
$$\langle \alpha|\beta\rangle=\sum_{n,m} a_n^*b_m \langle e_n|e^\prime_m\rangle$$

如果有幸两者的基底是一样且归一化的：
$$\langle \alpha|\beta\rangle=\sum_n a_n^*b_n $$
...
$$\begin{aligned}\hat{Q}_{\alpha\beta}&=\langle \psi_\alpha|\hat{Q} |\psi_\beta \rangle\\&=\langle \hat{S}^\dagger\psi_m|\hat{Q} |\hat{S}\psi_n\rangle\\&=\hat{S}^\dagger\hat{Q}_{mn}\hat{S}\end{aligned}$$
...
$$\hat{P}(\dfrac{|p\rangle\pm|-p\rangle}{\sqrt2})=\frac{1}{\sqrt2}(-|p\rangle\pm|p\rangle)=\pm(\dfrac{|p\rangle\pm|-p\rangle}{\sqrt2})$$
